...
const interImg = document.getElementById("interpretImg");

const points = L.geoJson(mapping, {
...
const lastMarker = Object.keys(points._layers).length - 1;
const markers = L.markerClusterGroup();

const groups = [...pointSet];
...
for (i in groups) {
  ...
  clusters[i] = new L.markerClusterGroup({
    iconCreateFunction: function (cluster) {
      ...
      if (cluster.getAllChildMarkers().length != 0) {
        var classGroup =
          "cluster " + cluster.getAllChildMarkers()[0].feature.properties.type;
      }
  ...
  for (y in Object.keys(points._layers)) {
    if (
      points._layers[Object.keys(points._layers)[y]].feature.properties.type ==
      clusters[i].type
    ) {
...
var overlayMaps = {
  SolarWay: solarway,
  "<img src='css/Images/access.svg' height='20px' width='20px'>  Access Points":
    clusters[0],
  ...
  "<img src='css/Images/interpret.svg' height='20px' width='20px'>  Info Boards":
    clusters[6],
  "<img src='css/Images/model.svg' height='20px' width='20px'>  Sculptures":
    clusters[7],
  Planets: clusters[8],
};
